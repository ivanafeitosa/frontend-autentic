...
async function cadastrar() {
  ...
  try {
    const response = await fetch(`http://localhost:8092/alunos/cadastro`, {
      method: 'POST',
      headers: {
    ...
    if (response.ok) {
      document.getElementById('msgSuccess').innerText = "Usuário cadastrado com sucesso";
      ...
      document.getElementById('msgError').innerText = data.message || "Erro ao cadastrar usuário";
      document.getElementById('msgError').style.display = 'block';
...
async function logar() {
  ...
  const response = await fetch(`http://localhost:8092/alunos/login`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
